...
import { Quiver, TableCellType } from "src/lib/Quiver"
import {
...
const typeToStyledComponent: Record<TableCellType, ComponentType> = {
...
export function getCellContentsGetter({
  ...
  return (columnIndex: number, rowIndex: number): CellContents => {
    ...
    const { type, id, classNames, content } = element.getCell(
      rowIndex,
      columnIndex
    )
    ...
    // Put it all together
    return { Component, contents: content, id, classNames }
  }
}
