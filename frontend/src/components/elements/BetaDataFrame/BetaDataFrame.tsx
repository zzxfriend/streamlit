...
export function BetaDataFrame({
  ...
  const [, nCols] = element.tableRowsAndColumns
  const {
    headerRows,
    headerColumns,
    dataRows,
  ...
  function getCellRenderer(
    ...
    return ({
      ...
      const { Component, classNames, contents, id } = cellContentsGetter(
        columnIndex,
        rowIndex
      )
  ...
  const getDataRowIndices = (nCols: number): number[] => {
    ...
    return element.getSortedDataRowIndices(
      sortColumn - headerColumns,
      sortAscending
    )
  }
  ...
  const { tableId, tableStyles } = element
