...
class BetaVegaLiteMixin:
    def beta_vega_lite_chart(
        ...
        **kwargs : any
        ...
        """
        vega_lite_chart_proto = BetaVegaLiteChartProto()
        marshall(
        ...
        return self.dg._enqueue("beta_vega_lite_chart", vega_lite_chart_proto)
...
def marshall(proto, data=None, spec=None, use_container_width=False, **kwargs):
    ...
    if "datasets" in spec:
        for k, v in spec["datasets"].items():
            ...
            arrow.marshall(dataset.data, v, "uuid")
        del spec["datasets"]
    ...
    if data is not None:
        arrow.marshall(proto.data, data, "uuid")
